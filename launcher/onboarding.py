...
1. :func:`status` — tell the GUI whether the user has a usable LLM
   ...
   ``mykey_template*.py``. Pure placeholders / no configs = onboarding
   needed.

2. :func:`save_minimal` — write the user-supplied key/base/model into
   ``<repo>/.env`` so :mod:`launcher.dotenv_shim` can synthesize a session
   on next start. We deliberately *don't* mutate ``mykey.py`` — that file
   may be hand-curated and we shouldn't surprise the user. ``.env`` is
   uniformly safe to append to: gitignored, parsed by the shim, no syntax.

Why not write ``mykey_local_override.py`` here? That file is owned by
the Qt launcher's "API configs" panel — keeping wizard writes in ``.env``
avoids two writers stepping on each other.
"""
...
def _env_path() -> str:
    """Allow tests to redirect via ``GA_ENV_FILE``; otherwise <repo>/.env."""
    override = os.environ.get("GA_ENV_FILE")
    if override:
        return override
...
def status() -> dict[str, Any]:
    ...
    Errors loading mykey.py are surfaced as ``needs_setup=True`` with a
    ``reason`` describing the load failure — don't leave the user staring
    at a blank screen because of a stack trace they won't see.
    """
    out: dict[str, Any] = {
        "needs_setup": False,
        "reason": "",
    ...
    # No real key found in mykey/override/json — see whether the dotenv
    # synthesizer would have produced something from the env. If yes,
    # this is *not* first-run; the user already configured via .env and
    # we just haven't reloaded.
    try:
