...
GenericAgent lets users configure multiple LLM endpoints (`mykey.py` plus
`mykey_local_override.py` written by the launcher). Currently:

- **At launcher level**: a "Profile" (cc-switch style) selects which configs
  are active for the whole process.
- **At session level**: each project carries an integer `llm_no` saved in
  ...
  `GA_LLM_NO`. The agent inside picks `self.llmclients[llm_no]` at startup.

This is fragile:

1. `llm_no` is an index into a list whose order depends on `mykey` scan
   order and which Profile is active. Same number means different things in
   different sessions.
...
When the agent subprocess starts, it receives `GA_LLM_CONFIG_NAME=<name>`
(in addition to the legacy `GA_LLM_NO`). On startup the agent:

1. If `GA_LLM_CONFIG_NAME` is set and matches a loaded client by `name`,
   select it.
2. Else fall back to `GA_LLM_NO`.
3. Else default to `0`.
...
- `launcher/project_manager.py::_spawn`: pass
  `env["GA_LLM_CONFIG_NAME"] = project.get("llm_config_name") or ""`.
- `agentmain.py`: `next_llm` accepts a name as an alternative to an index;
  startup logic prefers `GA_LLM_CONFIG_NAME` over `GA_LLM_NO`.
