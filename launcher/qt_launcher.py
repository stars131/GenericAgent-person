...
class ApiConfigTab(QWidget):
    ...
    def _build_ui(self):
        ...
        layout.addWidget(QLabel("API 配置 — 多渠道凭据 + Profile 切换"))
        ...
        for text, fn in (("新建", self.new_profile),
                         ("改名", self.rename_profile_ui),
                         ("删除", self.delete_profile_ui),
                         ("成员", self.edit_profile_members)):
            btn = QPushButton(text)
            btn.clicked.connect(fn)
...
class QtLauncher(QMainWindow):
    def __init__(self, base_dir=BASE_DIR):
        ...
        self.setWindowTitle(f"GenericAgent  v{APP_VERSION}")
    ...
    def _show_about(self):
        QMessageBox.about(self, "关于 GenericAgent",
                          f"<h3>GenericAgent</h3>"
