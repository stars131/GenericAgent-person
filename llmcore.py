...
def _load_mykeys():
    global _mykey_paths
    # ``.env`` loads first so that mykey.py code can ``os.environ.get(...)``
    # values from it without needing python-dotenv. Failures are silently
    # swallowed — the shim is best-effort.
    try:
    ...
    # Last-resort fallback: synthesize from env vars (loaded above from .env
    # if present). Lets a user run with just ``OPENAI_API_KEY`` exported, no
    # mykey.py needed — useful for first-run / containers.
    try:
        from launcher import dotenv_shim as _denv
        synth = _denv.synthesize_mykeys()
    except Exception:
        synth = {}
    if synth:
        _mykey_paths = ['<env>']
...
def _record_usage(usage, api_mode):
    ...
    if api_mode == 'responses':
        ...
        _accumulate_usage(api_mode, input_tokens=inp, output_tokens=out, cache_read=cached)
        ...
        _accumulate_usage(api_mode, input_tokens=inp, output_tokens=out,
                          cache_creation=ci, cache_read=cr)
