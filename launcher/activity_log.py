...
Design goals:
  ...
  * Zero cost when disabled (env ``GA_ACTIVITY_LOG_OFF=1``) — single bool check.
...
def _disabled() -> bool:
    return os.environ.get("GA_ACTIVITY_LOG_OFF", "") in ("1", "true", "yes")
...
def activity_dir() -> str:
    override = os.environ.get("GA_ACTIVITY_LOG_DIR")
    if override:
        return override
...
def _classify_outcome(exit_reason: dict[str, Any] | None) -> str | None:
    if not exit_reason:
        return None
