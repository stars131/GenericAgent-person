...
2. :func:`synthesize_mykeys` — turn well-known API env vars
   (``OPENAI_API_KEY``, ``ANTHROPIC_API_KEY``, plus ``GA_``-prefixed
   alternates) into the dict shape :mod:`llmcore` expects from a real
   ``mykey.py``. Used as a last-resort fallback when no ``mykey.py`` /
   ``mykey.json`` exists, so a user can drop a ``.env`` in the repo root
   and start the agent without writing any Python.

Why no external ``python-dotenv`` dependency? GenericAgent's design rule is
...
# first-match prefix logic (we strip ``GA_`` before lookup).
...
def _default_env_path() -> str:
    """``<repo_root>/.env``. ``launcher/`` sits one level below the root."""
...
#   ``GA_*`` prefixed variants take priority for users who don't want their
#   GenericAgent config to leak into other tools.
#
...
def synthesize_mykeys(env: dict[str, str] | None = None) -> dict[str, Any]:
    ...
    that as "no fallback available" and re-raises the original
    ``mykey.py not found`` error.
    """
    if env is None:
        env = dict(os.environ)
    out: dict[str, Any] = {}
    ...
    anth_key = _pick(env, "GA_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")
    if anth_key:
        anth_base = _pick(env, "GA_ANTHROPIC_BASE_URL", "ANTHROPIC_BASE_URL") or "https://api.anthropic.com"
        anth_model = _pick(env, "GA_ANTHROPIC_MODEL", "ANTHROPIC_MODEL") or "claude-opus-4-7"
        cfg = {
            "name": "anthropic-env",
            "apikey": anth_key,
            "apibase": anth_base,
    ...
    oai_key = _pick(env, "GA_OPENAI_API_KEY", "OPENAI_API_KEY")
    if oai_key:
        oai_base = _pick(env, "GA_OPENAI_BASE_URL", "OPENAI_BASE_URL") or "https://api.openai.com/v1"
        oai_model = _pick(env, "GA_OPENAI_MODEL", "OPENAI_MODEL") or "gpt-5.4"
